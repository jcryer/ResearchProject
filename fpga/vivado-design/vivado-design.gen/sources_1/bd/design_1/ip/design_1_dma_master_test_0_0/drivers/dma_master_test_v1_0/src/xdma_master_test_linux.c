...
typedef struct {
    ...
    XDma_master_test_uio_map maps[ MAX_UIO_MAPS ];
} XDma_master_test_uio_info;

/***************** Variable Definitions **************************************/
static XDma_master_test_uio_info uio_info;
...
static int uio_info_read_name(XDma_master_test_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDma_master_test_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDma_master_test_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDma_master_test_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDma_master_test_Initialize(XDma_master_test *InstancePtr, const char* InstanceName) {
	XDma_master_test_uio_info *InfoPtr = &uio_info;
